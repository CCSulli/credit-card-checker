...
const validateCred = cardNumber => { 
    ...
    const lastNumberRemoved = cardNumber.slice(0, -1); 
    const lastNumber = cardNumber.slice(-1);

    let reversed = Array.from(lastNumberRemoved.reverse());    
    ...
    let sum = (reversed.reduce((accumulator, value) => {
...
const findInvalidCards = creditCards => {
    for (let i = 0; i < creditCards.length - 1; i++) {
        let validation = validateCred(creditCards[i]);
        if (validation === false) {
...
const idInvalidCardCompanies = arr => {  
    ...
    for (let i = 0; i < arr.length; i++) {  
        if (arr[i][0] === 3 && companies.includes('Amex') === false) { 
            companies.push('Amex');
